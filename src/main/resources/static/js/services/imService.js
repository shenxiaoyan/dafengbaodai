...
//帐号模式，0-表示独立模式，1-表示托管模式
    ...
    var sdkAppID = 1400034003;
    var accountType = 13577;
    //当前用户身份
    var loginInfo = {
...
//                    msgflow.scrollTop = 1;
//                    $rootScope.message_loading = '正在载入历史消息';
//                    if (!isDigested) {
//            isDigested = true;
//            $rootScope.$digest();
//        }
...
//    };
    ...
    //joshua 发送群消息
    this.send_c2c_message = function (toAccount, msgtosend) {
        sendC2cMsg(toAccount, msgtosend);
    };
...
//聊天页面增加一条消息
    function addMsg(msg, prepend) {
        ...
        //组装头像
        if (msg.isSend) {//如果是自己发的消息
            if (loginInfo.headurl) {
                msg.fromAccountImage = loginInfo.headurl;
            } else {
                msg.fromAccountImage = 'img/me.jpg';
            }
        } else {//如果别人发的消息
            var info = infoMap[skey];
            if (info && info.name) {
                msg.fromAccountNick = info.name;
            }
            //获取头像
            if (info && info.image) {
                msg.fromAccountImage = info.image;
            }
        }


        if (msg.elems[0] && msg.elems[0].content && msg.elems[0].content.ext) {
            //joshua 问题：没有考虑一个消息体有多个元素的情况；
            var extString = msg.elems[0].content.ext;
            // console.log(extString);
            if (typeof extString === "string" && extString.indexOf('type') > -1) { //joshua 以后要删除，因为调时非josn时试存在其它消息
                msg.elems[0].content.ext = JSON.parse(extString);
            }
        }

        if(msg.fromAccount  === "admin") {
            $rootScope.notices[sessType + msg.seq] = msg;
            $rootScope.toasterSuccess("通知！","您有一条新的通知，请注意查收！");
        }else{
            $rootScope.sessions[skey].msgs[sessType + msg.seq] = msg;
            $rootScope.toasterInfo("来消息了！","有人联系你了，快去看看吧！");
        }

        //处理会议列表里最新消息
        $rootScope.sessions[skey].MsgShow = convertMsgtoTip(msg);
        if (!isDigested) {
            isDigested = true;
            $rootScope.$digest();
        }
...
//joshua：把消息转换成最后一条显示消息（会话列表页）
    function convertMsgtoTip(msg) {
                ...
                function (resp) {
                    ...
                    if (resp.FriendNum > 0) {
                        ...
                        for (var i = 0; i < count; i++) {
                            ...
                            var friend_image = friends[i].Info_Account;
                            if (friend_image == '') {
...
//从我的好友列表中给好友发消息
    function sendC2cMsg(toAccount, msgtosend) {
...
//删除群组成员
    this.deleteGroupMember = function (group_id, members) {
        if (!confirm("确定移除该成员吗？")) {
            return;
        }
        var options = {
...
//邀请好友加群
    this.addGroupMember = function (group_id, members) {
        var options = {
            'GroupId': group_id,
            'MemberList': members
        };
    ...
    var getPrePageGroupHistoryMsgs = function (cbOk) {
        ...
        webim.syncGroupMsgs(
                ...
                function (msgList) {
                    if (msgList.length == 0) {
                        //joshua: 没有历史消息了
                        $rootScope.message_loading = '已经没有其它更早消息了';
                        if (!isDigested) {
                            isDigested = true;
                            $rootScope.$digest();
                        }
...
//检查文件类型和大小
    function checkFile(file) {
        ...
        var ext = file.name.substr(file.name.lastIndexOf(".") + 1).toLowerCase();//获得文件后缀名
