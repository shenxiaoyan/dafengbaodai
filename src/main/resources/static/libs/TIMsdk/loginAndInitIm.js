...
function addMsg(msg, prepend) {
    ...
    //金杭
    if (msg.elems[0].content.ext) {
        msg.elems[0].content.ext = JSON.parse(msg.elems[0].content.ext);
    }

    // console.log(msg)
    // console.log(sessMap["C2Cadmin"].unread());
    //获取会话类型，目前只支持群聊
    ...
    if (typeof msg.elems[0].content.ext === "object") {
        var href = "#/" + msg.elems[0].content.ext.entityType + "/" + msg.elems[0].content.ext.entityName + "/" + msg.elems[0].content.ext.entityId + "/profile/info";
    }

    var msgPre = document.createElement("pre");
    msgPre.style.backgroundColor = "#FFF";
    ...
    //昵称  消息时间
    if (webim.Tool.formatText2Html(fromAccountNick) === "admin") {
        msghead.innerHTML = "<img class='headurlClass' src='" + fromAccountImage + "'>" + "&nbsp;&nbsp;" + webim.Tool.formatText2Html("系统通知&nbsp;&nbsp;" + webim.Tool.formatTimeStamp(msg.getTime()));
    } else if (webim.Tool.formatText2Html(fromAccountNick) === "oEgy-v7HIXsOdEm8n9XklfUZjyuo") {

        var handleHtml = '<div class="m-l-sm btn-group">' +
            '  <button type="button" class="btn btn-sm btn-info dropdown-toggle" data-toggle="dropdown">' +
            '    标的操作 <span class="caret"></span>' +
            '  </button>' +
            '  <ul class="dropdown-menu">' +
            '    <li><a href="' + href + '" onclick="$(\'#message_container\').css(\'display\',\'none\');$(\'#common_container\').css(\'display\',\'block\');">查看详情</a></li>' +
            '  </ul>' +
            '</div>';

        msghead.innerHTML = "<img class='headurlClass' src='" + fromAccountImage + "'>" + "&nbsp;&nbsp;" + webim.Tool.formatText2Html("标的分配通知" + "&nbsp;&nbsp;" + webim.Tool.formatTimeStamp(msg.getTime())) + handleHtml;

    } else if (webim.Tool.formatText2Html(fromAccountNick) === "o1fYG1D9ScQyv3tCDVs5cPaoX7ic") {

        var distEntityId = msg.elems[0].content.ext.entityId;
        ...
        var handleHtml = '<div class="m-l-sm btn-group">' +
            ...
            '  <ul class="dropdown-menu">' +
            '    <li><a href="' + href + '" onclick="$(\'#message_container\').css(\'display\',\'none\');$(\'#common_container\').css(\'display\',\'block\');">查看详情</a></li>' +
            '    <li><a onclick="$(\'#global_distribution\').modal(\'show\');window.distEntityId.value = ' + distEntityId + ';">分配业务员</a></li>' +
            '  </ul>' +
            '</div>';

        msghead.innerHTML = "<img class='headurlClass' src='" + fromAccountImage + "'>" + "&nbsp;&nbsp;" + webim.Tool.formatText2Html("新标的通知" + "&nbsp;&nbsp;" + webim.Tool.formatTimeStamp(msg.getTime())) + handleHtml;
    } else {
    ...
    msg.elems[0].content.ext = JSON.stringify(msg.elems[0].content.ext);
    ...
    switch (subType) {
        ...
        case webim.GROUP_MSG_SUB_TYPE.COMMON://群普通消息
            msgPre.innerHTML = (msg.elems[0].content.data ? msg.elems[0].content.data : convertMsgtoHtml(msg));
        ...
        function (resp) {
            ...
            if (resp.SessionItem && resp.SessionItem.length > 0) {
                ...
                for (var i in resp.SessionItem) {
                    ...
                    var sessType, typeZh, sessionId, sessionNick = '', sessionImage = '', senderId = '',
                        senderNick = '';
...
function onMsgNotify(newMsgList) {
    ...
    for (var j in newMsgList) {//遍历新消息
        ...
        if ($("#message_container").css("display") === "none") {
            $("#message_un_read").css("display", "block");
        } else {
        ...
        if (newMsg.elems[0].content.ext) {
            newMsg.elems[0].content.ext = JSON.parse(newMsg.elems[0].content.ext);
            toastr.success('<a onclick="$(\'#message_un_read\').css(\'display\',\'none\');$(\'#message_container\').css(\'display\',\'none\');$(\'#common_container\').css(\'display\',\'block\');" href="#/' + newMsg.elems[0].content.ext.entityType + '/' + newMsg.elems[0].content.ext.entityName + 's/' + newMsg.elems[0].content.ext.entityId + '/profile/info">您收到一条新的系统通知</a>', '<a onclick="$(\'#message_un_read\').css(\'display\',\'none\');$(\'#message_container\').css(\'display\',\'none\');$(\'#common_container\').css(\'display\',\'block\');" href="#/' + newMsg.elems[0].content.ext.entityType + '/' + newMsg.elems[0].content.ext.entityName + 's/' + newMsg.elems[0].content.ext.entityId + '/profile/info">通知！</a>');
        } else {
            toastr.success("您收到一条新的信息", "通知！");
        }
